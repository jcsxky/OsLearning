...
extern unsigned get_pci_cfg_reg(int bus, int dev, int func, int reg);
struct list_head{
...
struct pci_dev* pcidevs[256][32][8];
typedef struct list_head list_head_t;
#define INIT_LIST_HEAD(l)       \
	do{                           \
		(l)->prev = (l)->next = l;  \
	}while(0)
#define MB2STRU(stru_type, mb_addr, mb_name) (stru_type *)( (unsigned long)(mb_addr)-  (unsigned long)&((stru_type *)0)->mb_name )
struct list_head pcidevs_root;
struct list_head pcidrvs_root;
...
struct net_device{
	...
	int 	(*tx_busy) (struct net_device *dev);
	u8 mac[7];
...
#define PCI_CONFIG_ADDR 0xCF8	/* register to choose pci device*, and choose one of 
								   256/4 registers */
/* format
31　：Enabled位。
23:16 ：总线编号。
...
10: 8 ：功能编号。
7: 2 ：配置空间寄存器编号。
1: 0 ：恒为“00”。这是因为CF8h、CFCh端口是32位端口。
*/
#define PCI_CONFIG_DATA 0xCFC	/*after PCI_CONFIG_ADDR written, we can investigate the 
								corresponding PCI-configuration room. totally 256 bytes*/
/*
